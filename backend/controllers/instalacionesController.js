...
class InstalacionesController {
  ...
  static async crear(req, res) {
    try {
      ...
      if (!cliente_id || !fecha_programada) {
        return ApiResponse.respuestaError(res, 'Los campos cliente_id y fecha_programada son obligatorios', 400);
      }

      // Validar formato de fecha
      if (!validarFecha(fecha_programada)) {
        return ApiResponse.respuestaError(res, 'El formato de fecha_programada debe ser YYYY-MM-DD', 400);
      }
      ...
      if (fechaProgramada <= ahora) {
        return ApiResponse.respuestaError(res, 'La fecha programada debe ser futura', 400);
      }

      // Validar coordenadas si se proporcionan
      if (coordenadas_lat && (coordenadas_lat < -90 || coordenadas_lat > 90)) {
        return ApiResponse.respuestaError(res, 'La latitud debe estar entre -90 y 90 grados', 400);
      }

      if (coordenadas_lng && (coordenadas_lng < -180 || coordenadas_lng > 180)) {
        return ApiResponse.respuestaError(res, 'La longitud debe estar entre -180 y 180 grados', 400);
      }

      // Validar costo de instalación
      if (costo_instalacion && !validarNumeroPositivo(costo_instalacion)) {
        return ApiResponse.respuestaError(res, 'El costo de instalación debe ser un número positivo', 400);
      }
      ...
      if (tipo_instalacion && !tiposPermitidos.includes(tipo_instalacion)) {
        return ApiResponse.respuestaError(res, `El tipo de instalación debe ser uno de: ${tiposPermitidos.join(', ')}`, 400);
      }
      ...
      if (equipos_instalados && (!Array.isArray(equipos_instalados) || equipos_instalados.length === 0)) {
        return ApiResponse.respuestaError(res, 'Los equipos instalados deben ser un array no vacío', 400);
      }

      const datosInstalacion = {
        cliente_id: parseInt(cliente_id),
        servicio_cliente_id: servicio_cliente_id ? parseInt(servicio_cliente_id) : null,
        instalador_id: instalador_id ? parseInt(instalador_id) : null,
        fecha_programada,
        hora_programada,
        direccion_instalacion: direccion_instalacion ? direccion_instalacion.trim() : null,
      ...
      return ApiResponse.respuestaExitosa(res, {
        instalacion,
      ...
      return ApiResponse.respuestaError(res, error.message || 'Error interno del servidor', 500);
    }
  }
  ...
  static async listar(req, res) {
    try {
      ...
      if (paginaNum < 1 || limiteNum < 1 || limiteNum > 100) {
        return ApiResponse.respuestaError(res, 'Parámetros de paginación inválidos. Página >= 1, límite entre 1 y 100', 400);
      }

      // Validar fechas si se proporcionan
      if (fecha_desde && !validarFecha(fecha_desde, 'YYYY-MM-DD')) {
        return ApiResponse.respuestaError(res, 'El formato de fecha_desde debe ser YYYY-MM-DD', 400);
      }

      if (fecha_hasta && !validarFecha(fecha_hasta, 'YYYY-MM-DD')) {
        return ApiResponse.respuestaError(res, 'El formato de fecha_hasta debe ser YYYY-MM-DD', 400);
      }

      // Validar estados permitidos
      const estadosPermitidos = ['programada', 'en_proceso', 'completada', 'cancelada', 'reagendada'];
      if (estado && !estadosPermitidos.includes(estado)) {
        return ApiResponse.respuestaError(res, `El estado debe ser uno de: ${estadosPermitidos.join(', ')}`, 400);
      }
      ...
      return ApiResponse.respuestaExitosa(res, resultado);

    } catch (error) {
      console.error('Error al listar instalaciones:', error);
      return ApiResponse.respuestaError(res, 'Error interno del servidor', 500);
    }
  }
  ...
  static async obtenerPorId(req, res) {
    try {
      ...
      if (!id || isNaN(parseInt(id))) {
        return ApiResponse.respuestaError(res, 'ID de instalación inválido', 400);
      }

      const instalacion = await Instalacion.obtenerPorId(parseInt(id));

      if (!instalacion) {
        return ApiResponse.respuestaNoEncontrado(res, 'Instalación no encontrada');
      }

      return ApiResponse.respuestaExitosa(res, { instalacion });
  ...
  static async actualizar(req, res) {
    try {
      ...
      if (datosActualizacion.fecha_programada) {
        if (!validarFecha(datosActualizacion.fecha_programada, 'YYYY-MM-DD')) {
          return ApiResponse.respuestaError(res, 'El formato de fecha_programada debe ser YYYY-MM-DD', 400);
        }
      }

      if (datosActualizacion.fecha_realizada) {
        if (!validarFecha(datosActualizacion.fecha_realizada, 'YYYY-MM-DD')) {
          return ApiResponse.respuestaError(res, 'El formato de fecha_realizada debe ser YYYY-MM-DD', 400);
        }
      }
  ...
  static async cambiarEstado(req, res) {
    try {
      ...
      if (!estado) {
        return ApiResponse.respuestaError(res, 'El estado es obligatorio', 400);
      }
      ...
      if (estado === 'completada') {
        if (!fecha_realizada) {
          return ApiResponse.respuestaError(res, 'La fecha de realización es obligatoria para completar la instalación', 400);
        }

        if (!validarFecha(fecha_realizada, 'YYYY-MM-DD')) {
          return ApiResponse.respuestaError(res, 'El formato de fecha_realizada debe ser YYYY-MM-DD', 400);
        }
  ...
  static async obtenerEstadisticas(req, res) {
    try {
      ...
      return ApiResponse.respuestaExitosa(res, { estadisticas });
  ...
  static async obtenerPendientesPorInstalador(req, res) {
    try {
      ...
      if (!instalador_id || isNaN(parseInt(instalador_id))) {
        return ApiResponse.respuestaError(res, 'ID de instalador inválido', 400);
      }
  ...
  static async reagendar(req, res) {
    try {
      ...
      if (!nueva_fecha) {
        return ApiResponse.respuestaError(res, 'La nueva fecha es obligatoria', 400);
      }

      if (!validarFecha(nueva_fecha, 'YYYY-MM-DD')) {
        return ApiResponse.respuestaError(res, 'El formato de nueva_fecha debe ser YYYY-MM-DD', 400);
      }
      ...
      const ahora = new Date();
      if (nuevaFecha <= ahora) {
        return ApiResponse.respuestaError(res, 'La nueva fecha debe ser futura', 400);
      }
